###### OS / Systems
from curses import noecho
import sys

from fastapi import Depends, Request
import pydash
from authorizations.utils import account_id_header, acl_enforce
from authorizations.constant import PermissionDenied
from routers.utils import is_alphanumeric_underscore
from routers.constant import NOT_ALPHA_NUMERIC_UNDERSCORE_STRING_RESPONSE

###### Parsers, Formats, Utils
import re
import csv
import json
import time
import logging


##### Typing
from pydantic import BaseModel
from typing import Union, Any, Dict, List

###### FastAPI
from APIRouter import APIRouter
from fastapi.responses import JSONResponse


###### Schema
class ToolSchema(BaseModel):
    name: str
    description: Union[str, None] = None
    icon: Union[str, dict, None] = None


JSONObject = Dict[str, Any]
JSONArray = List[Any]
JSONStructure = Union[JSONArray, JSONObject, Any]
######


###### Blue
from blue.platform import Platform
from blue.tools.registry import ToolRegistry

###### Properties
from blue.properties import PROPERTIES
from settings import ACL

### Assign from platform properties
platform_id = PROPERTIES["platform.name"]
prefix = 'PLATFORM:' + platform_id
tool_registry_id = PROPERTIES["tool_registry.name"]
PLATFORM_PREFIX = f'/blue/platform/{platform_id}'

###### Initialization
p = Platform(id=platform_id, properties=PROPERTIES)
tool_registry = ToolRegistry(id=tool_registry_id, prefix=prefix, properties=PROPERTIES)

##### ROUTER
router = APIRouter(prefix=f"{PLATFORM_PREFIX}/registry/{tool_registry_id}/tools", dependencies=[Depends(account_id_header)])

# set logging
logging.getLogger().setLevel("INFO")


write_all_roles = ACL.get_implicit_users_for_permission('tool_registry', 'write_all')
write_own_roles = ACL.get_implicit_users_for_permission('tool_registry', 'write_own')


def server_acl_enforce(request: Request, server: dict, write=False, throw=True):
    user_role = request.state.user['role']
    uid = request.state.user['uid']
    allow = False
    if write and user_role in write_all_roles:
        allow = True
    elif write and user_role in write_own_roles:
        if pydash.objects.get(server, 'created_by', None) == uid:
            allow = True
    if throw and not allow:
        raise PermissionDenied
    return allow


#############
@router.get("/")
def get_tools(request: Request):
    acl_enforce(request.state.user['role'], 'tool_registry', 'read_all')
    results = tool_registry.list_records()
    return JSONResponse(content={"results": results})


@router.get("/search")
def search_tools(request: Request, keywords, approximate: bool = False, hybrid: bool = False, type: str = None, scope: str = None, page: int = 0, page_size: int = 10):
    acl_enforce(request.state.user['role'], 'tool_registry', 'read_all')
    results = tool_registry.search_records(keywords, type=type, scope=scope, approximate=approximate, hybrid=hybrid, page=page, page_size=page_size)
    return JSONResponse(content={"results": results})


@router.get("/{server_name}")
def get_server(request: Request, server_name):
    acl_enforce(request.state.user['role'], 'tool_registry', 'read_all')
    result = tool_registry.get_server(server_name)
    return JSONResponse(content={"result": result})


@router.post("/{server_name}")
def add_server(request: Request, server_name, data: ToolSchema):
    if not is_alphanumeric_underscore(server_name):
        return NOT_ALPHA_NUMERIC_UNDERSCORE_STRING_RESPONSE
    server = tool_registry.get_server(server_name)
    # if server already exists, return 409 conflict error
    if not pydash.is_empty(server):
        return JSONResponse(content={"message": f"\"{server_name}\" already exists"}, status_code=409)
    acl_enforce(request.state.user['role'], 'tool_registry', ['write_all', 'write_own'])
    # TODO: properties
    tool_registry.register_server(server_name, request.state.user['uid'], description=data.description, properties={}, rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})


@router.put("/{server_name}")
def update_server(request: Request, server_name, data: ToolSchema, sync: bool = False, recursive: bool = False):
    server = tool_registry.get_server(server_name)
    server_acl_enforce(request, server, write=True)
    # TODO: properties
    tool_registry.update_server(server_name, description=data.description, icon=data.icon, properties={}, rebuild=True)
    if sync:
        tool_registry.sync_server(server_name, recursive=recursive, rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})


@router.delete("/{server_name}")
def delete_server(request: Request, server_name):
    server = tool_registry.get_server(server_name)
    server_acl_enforce(request, server, write=True)
    tool_registry.deregister_server(server_name, rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})


##### properties
@router.get("/{server_name}/properties")
def get_server_properties(request: Request, server_name):
    acl_enforce(request.state.user['role'], 'tool_registry', 'read_all')
    results = tool_registry.get_server_properties(server_name)
    return JSONResponse(content={"results": results})


@router.get("/{server_name}/property/{property_name}")
def get_server_property(request: Request, server_name, property_name):
    acl_enforce(request.state.user['role'], 'tool_registry', 'read_all')
    result = tool_registry.get_server_property(server_name, property_name)
    return JSONResponse(content={"result": result})


@router.post("/{server_name}/property/{property_name}")
def set_server_property(request: Request, server_name, property_name, property: JSONStructure):
    server_db = tool_registry.get_server(server_name)
    server_acl_enforce(request, server_db, write=True)
    tool_registry.set_server_property(server_name, property_name, pydash.objects.get(property, [property_name], None), rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})


@router.delete("/{server_name}/property/{property_name}")
def delete_server_property(request: Request, server_name, property_name):
    server_db = tool_registry.get_server(server_name)
    server_acl_enforce(request, server_db, write=True)
    tool_registry.delete_server_property(server_name, property_name, rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})


@router.get("/{server_name}/tools")
def get_server_tools(request: Request, server_name):
    acl_enforce(request.state.user['role'], 'tool_registry', 'read_all')
    results = tool_registry.get_server_tools(server_name)
    return JSONResponse(content={"results": results})


@router.get("/{server_name}/tool/{tool_name}")
def get_server_tool(request: Request, server_name, tool_name):
    acl_enforce(request.state.user['role'], 'tool_registry', 'read_all')
    result = tool_registry.get_server_tool(server_name, tool_name)
    return JSONResponse(content={"result": result})


@router.post("/{server_name}/tool/{tool_name}")
def add_server_tool(request: Request, server_name, tool_name, data: ToolSchema):
    if not is_alphanumeric_underscore(tool_name):
        return NOT_ALPHA_NUMERIC_UNDERSCORE_STRING_RESPONSE
    server = tool_registry.get_server(server_name)
    server_acl_enforce(request, server, write=True)
    # TODO: properties
    tool_registry.register_server_tool(server_name, tool_name, description=data.description, properties={}, rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})


@router.put("/{server_name}/tool/{tool_name}")
def update_server_tool(request: Request, server_name, tool_name, data: ToolSchema, sync: bool = False, recursive: bool = False):
    server = tool_registry.get_server(server_name)
    server_acl_enforce(request, server, write=True)
    # TODO: properties
    tool_registry.update_server_tool(server_name, tool_name, description=data.description, properties={}, rebuild=True)
    if sync:
        tool_registry.sync_server_tool(server_name, tool_name, recursive=recursive, rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})


@router.delete("/{server_name}/tool/{tool_name}")
def delete_server_tool(request: Request, server_name, tool_name):
    server = tool_registry.get_server(server_name)
    server_acl_enforce(request, server, write=True)
    tool_registry.deregister_server_tool(server_name, tool_name, rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})


@router.get("/{server_name}/tool/{tool_name}/properties")
def get_server_tool_properties(request: Request, server_name, tool_name):
    acl_enforce(request.state.user['role'], 'tool_registry', 'read_all')
    results = tool_registry.get_server_tool_properties(server_name, tool_name)
    return JSONResponse(content={"results": results})


@router.get("/{server_name}/tool/{tool_name}/property/{property_name}")
def get_server_tool_property(request: Request, server_name, tool_name, property_name):
    acl_enforce(request.state.user['role'], 'tool_registry', 'read_all')
    result = tool_registry.get_server_tool_property(server_name, tool_name, property_name)
    return JSONResponse(content={"result": result})


@router.post("/{server_name}/tool/{tool_name}/property/{property_name}")
def set_server_tool_property(request: Request, server_name, tool_name, property_name, property: JSONStructure):
    server_db = tool_registry.get_server(server_name)
    server_acl_enforce(request, server_db, write=True)
    tool_registry.set_server_tool_property(server_name, tool_name, property_name, pydash.objects.get(property, [property_name], None), rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})


@router.delete("/{server_name}/tool/{tool_name}/property/{property_name}")
def delete_server_tool_property(request: Request, server_name, tool_name, property_name):
    server_db = tool_registry.get_server(server_name)
    server_acl_enforce(request, server_db, write=True)
    tool_registry.delete_server_tool_property(server_name, tool_name, property_name, rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})


### sync entities
@router.put('/{server_name}/sync')
def sync_server(request: Request, server_name, recursive: bool = False):
    server = tool_registry.get_server(server_name)
    server_acl_enforce(request, server, write=True)
    tool_registry.sync_server(server_name, recursive=recursive, rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})


@router.put("/{server_name}/tool/{tool_name}/sync")
def sync_server_tool(request: Request, server_name, tool_name, recursive: bool = False):
    server = tool_registry.get_server(server_name)
    server_acl_enforce(request, server, write=True)
    tool_registry.sync_server_tool(server_name, tool_name, recursive=recursive, rebuild=True)
    # save
    tool_registry.dump("/blue_data/config/" + tool_registry_id + ".tools.json")
    return JSONResponse(content={"message": "Success"})
