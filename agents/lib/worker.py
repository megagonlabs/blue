...
class Worker():
    def __init__(self, input_stream, name="WORKER", id=None, sid=None, cid=None, prefix=None, suffix=None, agent=None, processor=None, session=None, properties={}):
    ...
    def _start_consumer_on_stream(self, input_stream):
        ...
        if input_stream:
            ...
            if self.session:
                self.session._init_stream_agent_data_namespace(input_stream, self.agent.name)

            consumer = Consumer(input_stream, name=self.name+"WORKER", prefix=self.cid, listener=lambda id, data : self.listener(id,data,input_stream), properties=self.properties)
    ...
    def set_data(self, key, value, stream=None):
        if self.session:
            stream = self._identify_stream(stream=stream)
            self.session.set_stream_agent_data(stream, self.sidid, key, value)
