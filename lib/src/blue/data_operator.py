...
class DataOperator(Tool):
    """
    DataOperator is a specialized Tool to perform data operations in Blue (JSON array data records).
    ...
    DataOperator doesn't listen to any data stream, it only processes data when called.
    """

    def __init__(self, name: str, description: str, properties: Dict[str, Any] = None, parameters: Dict[str, Any] = None, validator: Callable = None, explainer: Callable = None):
        """
        Initialize a DataOperator.
        ...
        # Set default parameters for data operators
        default_parameters = {"input_data": {"type": "list[list[dict]]", "description": "List of data groups, each containing JSON array of records", "required": True}}
        ...
        super().__init__(name=name, description=description, properties=self.properties, function=self._execute_operator, parameters=merged_parameters, validator=validator, explainer=explainer)
    ...
    def _initialize(self, properties=None):
        """Initialize the DataOperator following the same pattern as Agent."""
        self._initialize_properties()
        self._update_properties(properties=properties)

    def _initialize_properties(self):
        """Initialize default properties for data operators."""
        self.properties = {}
        ...
        # Processing properties (TODO: implement corresponding logic)
    ...
    def _default_validator(self, params: Dict[str, Any]) -> bool:
        """Default validator for data operator parameters."""
        # Get merged properties for validation
        ...
        # Validate input_data specifically (always required for data operators)
    ...
    def _default_explainer(self, output: Any, params: Dict[str, Any]) -> Dict[str, Any]:
        """Default explainer for data operator output."""
    ...
    def _execute_operator(self, **kwargs) -> List[Dict[str, Any]]:
        """
        Main entry point for data operator execution.
        ...
        try:
            ...
            if error_handling == "fail":
                ...
                self.logger.error(f"Error in {self.name}, skipping: {str(e)}")
                return []
            else:  # skip
                self.logger.warning(f"Error in {self.name}, skipping: {str(e)}")
                return []
    ...
    def get_data_statistics(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if not data:
            return {"record_count": 0, "fields": [], "field_types": {}}
        ...
        return {"record_count": len(data), "fields": list(all_fields), "field_types": field_types}

    def get_data_sources_statistics(self, input_data: List[List[Dict[str, Any]]]) -> Dict[str, Any]:
        """Get statistics about all data sources."""
        statistics = {"total_sources": len(input_data), "sources": {}}
    ...
    def aggregate_records(self, data: List[Dict[str, Any]], group_by: str, aggregate_func: Callable, value_field: str) -> List[Dict[str, Any]]:
        ...
        for group_key, values in groups.items():
            result.append({group_by: group_key, f"{value_field}_{aggregate_func.__name__}": aggregate_func(values)})
